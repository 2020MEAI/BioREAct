...
class HandshakingTaggingScheme(object):
    ...
    def __init__(self, rel2id, max_seq_len):
        ...
        self.tag2id_ent = {
            "O": 0,
            "ENT-H2T": 1, # entity head to entity tail  实体标签两种 0、1
        }
        ...
        self.tag2id_head_rel = {
            ...
            "REL-OH2SH": 2, # object head to subject head  关系标签3种
        }
        ...
        # 没有优化前的完整二维矩阵，上位三角每个元素储存着上三角铺平序列的相对应的位置序号
    ...
    def get_spots(self, sample):
        ...
        for rel in sample["relation_list"]:
            ...
            # 实体头部存储，[关系类别，实体_1头部，实体_2头部，关系标签（1，2）]
...
class TPLinkerBert(nn.Module):
    def __init__(self, encoder, 
        ...
        self.ent_fc = nn.Linear(hidden_size, 2) # 实体预测，维度是2（标签0和1）
        self.head_rel_fc_list = [nn.Linear(hidden_size, 3) for _ in range(rel_size)] # 关系头部的一个全连接（标签有三个0，1，2）每个关系有一个独立的MLP层
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        last_hidden_state_sent = context_outputs[1]
...
#             shaking_hiddens4rel = shaking_hiddens
        ...
        ent_shaking_outputs = self.ent_fc(shaking_hiddens4ent)# 实体抽取mlp层的预测结果
        ...
        return ent_shaking_outputs, head_rel_shaking_outputs, tail_rel_shaking_outputs, last_hidden_state_sent
